...
import pe.com.acopio.service.PrinterService;
...
public class AcopioController {
    ...
    private void handleGuardarAcopio() {
        ...
        try {
            ...
            acopio.setUsuario(sessionManager.getUsuarioActual());
        ...
        try {
            ...
            if (printerService.isPrinterAvailable()) {
                try {
                    ...
                    if (ReportAlert.showConfirmation("Imprimir",
                            "¿Desea imprimir el voucher en: " + printerName + "?")) {

                        printerService.printToDefaultPrinter(jasperPrint);
                        ReportAlert.showSuccess("Éxito", "Voucher enviado a la impresora: " + printerName);
                    } else {
                        // Usuario canceló, mostrar en pantalla
                        ReportAlert.showReport(jasperPrint, "Voucher de Acopio");
                    }
                } catch (Exception printError) {
                    // Error al imprimir, ofrecer ver en pantalla
                    boolean verEnPantalla = ReportAlert.showConfirmation(
                            "Error de Impresión",
                            "No se pudo imprimir: " + printError.getMessage() +
                                    "\n\n¿Desea ver el voucher en pantalla?");
                ...
                // No hay impresora, mostrar en pantalla directamente
                ReportAlert.showWarning("Sin Impresora",
                        "No se detectó ninguna impresora.\nSe mostrará el voucher en pantalla.");
                ReportAlert.showReport(jasperPrint, "Voucher de Acopio");
            }
    ...
    private void actualizarTotalGeneral() {
        ...
        lblTotalGeneral.setText(total.setScale(2, RoundingMode.HALF_UP).toString());
    }
